...
export default async function LeaderStudentDetailPage({ params }: PageProps) {
  const user = await getCurrentUserOrRedirect();
  requireRole(user, ["LEADER"]);
  const { id } = await params;
  const studentUserId = Number(id);
  if (!(await canViewStudent(user, studentUserId))) redirect("/leader/groups");
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">{`${student.email} · leader view`}</p>
      </div>
      <StudentDetail
        student={student}
